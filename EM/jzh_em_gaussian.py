...
def init_model(args):
    if args.cluster_num:
        ...
        lambdas = np.random.random(lambdas.shape)
        lambdas = lambdas / np.sum(lambdas)
        mus = np.random.random(mus.shape)
        sigma = np.random.random(sigma.shape)
...
def train_model(model, train_xs, dev_xs, args):
    ...
    xs = train_xs if args.nodev else dev_xs
    N = xs.shape[0]
    ...
    for _ in range(args.iterations):
        ...
        for x in xs:
            ...
            for i in range(args.cluster_num):
                ki = lambdas[i] * multivariate_normal(mean = mus[i], cov = sigmas[i]).pdf(x)
                ksi.append(ki)    
            ksi = ksi / np.sum(ksi)
            ks.append(ksi)
        ks = np.asarray(ks)
        ...
        # M-step
        for i in range(args.cluster_num):
            lambdas[i] = np.sum(ks[:,i]) / N
            mus[i] = np.dot(ks[:,i], xs) / np.sum(ks[:,i])
            sigmas[i] =  np.dot((xs-mus[i]).T, np.diag(ks[:,i])).dot(xs-mus[i]) / np.sum(ks[:,i])
...
def average_log_likelihood(model, data, args):
    ...
    for x in data:
        p = 0.0
        for i in range(args.cluster_num):
            p += lambdas[i] * multivariate_normal(mean = mus[i], cov = sigmas[i]).pdf(x)
        ps.append(p)
    ll = np.sum(np.log(ps)) / N
